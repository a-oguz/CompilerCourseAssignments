...
public:
	...
	size_t start_pos = 0;
    	while((start_pos = temp.find("\\n")) != std::string::npos) {
       		temp.replace(start_pos, 2, "\n");
       	}
start_pos = 0;
    	while((start_pos = temp.find("\\r")) != std::string::npos) {
       		temp.replace(start_pos, 2, "\r");
       	}
start_pos = 0;
    	while((start_pos = temp.find("\\v")) != std::string::npos) {
       		temp.replace(start_pos, 2, "\v");
       	}
start_pos = 0;
    	while((start_pos = temp.find("\\b")) != std::string::npos) {
       		temp.replace(start_pos, 2, "\b");
       	}
start_pos = 0;
    	while((start_pos = temp.find("\\a")) != std::string::npos) {
       		temp.replace(start_pos, 2, "\a");
       	}
start_pos = 0;
    	while((start_pos = temp.find("\\f")) != std::string::npos) {
       		temp.replace(start_pos, 2, "\f");
       	}
start_pos = 0;
    	while((start_pos = temp.find("\\t")) != std::string::npos) {
       		temp.replace(start_pos, 2, "\t");
       	}
start_pos = 0;
    	while((start_pos = temp.find("\\\"")) != std::string::npos) {
...
public:
	...
	llvm::Value *Codegen() { throw runtime_error("FieldDecl");}
